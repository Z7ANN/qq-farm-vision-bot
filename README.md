...
> **注意**：当前项目开发者等级为 26 级，商店种子模板仅采集到 26 级以下的作物。如果你的等级超过 26 级，需要自行采集高等级作物的 `shop_作物名.png` 模板，否则自动购买功能可能无法识别高等级种子。
...
打开微信小程序中的 QQ 经典农场，然后运行模板采集工具：

```bash
# 推荐：PyQt6 GUI 版本（无需 OpenCV GUI 支持）
python tools/template_collector_gui.py

# 或使用 OpenCV 版本（需要完整的 opencv-python）
...
2. 鼠标左键拖拽框选模板区域
3. 点击"保存模板"按钮或按 `S` 键
4. 输入模板名称（如 `btn_harvest`）
5. 点击"重新截屏"或按 `R` 键刷新画面
6. 按 `Q` 键退出
...
| `btn_friend_help.png` | 好友求助按钮 | 农场主页右下角 |
...
│
└── tools/                      # 辅助工具
    ├── template_collector.py   # OpenCV 版模板采集
    ├── template_collector_gui.py # PyQt6 GUI 版模板采集（推荐）
